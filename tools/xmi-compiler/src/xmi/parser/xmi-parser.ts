...
export class XmiParser {
    ...
    this.parser = new XMLParser({
      ...
      isArray: (tagName: string) => {
        return [
          ...
          'memberEnd', 'ownedLiteral', 'generalization', 'taggedValue',
        ].includes(tagName)
      },
  ...
  /** Parse an XMI file from disk */
  parseFile(filePath: string): ParsedXmi {
  ...
  private parseClass(el: XmiPackagedElement, idToName: Record<string, string>, packagePath: string[]): ParsedClass {
    ...
    return { id, name, isAbstract, attributes, generalizations, associationEndIds, packagePath }
  }
  ...
  private parseDataType(el: XmiPackagedElement, packagePath: string[]): ParsedDataType {
    ...
    return { id, name, baseTypeId, tags, packagePath }
  }
}
